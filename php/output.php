...
function get_the_recipe() {
	...
	if( has_recipress_recipe() && recipress_output() ) {
		...
		// create the array
		$recipe['before'] = '<div class="' . recipress_options( 'theme' ) . '" id="recipress_recipe" itemscope itemtype="http://data-vocabulary.org/Recipe">';
		...
		if( recipress_recipe( 'yield' ) )
			$recipe['yield'] = '<li><b>' . __( 'Yield:', 'recipress' ) . '</b> <span class="yield" itemprop="yield">' . recipress_recipe( 'yield' ) . '</span></li>';
